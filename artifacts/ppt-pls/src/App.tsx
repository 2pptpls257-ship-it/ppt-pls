    ...
    <header className="relative z-20 border-b border-[hsl(var(--primary)/.14)] bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-10">
        ...
        <nav className="hidden items-center gap-8 text-[13px] text-[hsl(var(--primary-foreground)/.68)] md:flex">
          <a
            href="#how-it-works"
            className="transition-colors hover:text-[hsl(var(--secondary))]"
          ...
          <a
            href="#order"
            className="rounded-full border border-[hsl(var(--secondary)/.55)] px-4 py-2 text-[hsl(var(--secondary))] transition-colors hover:bg-[hsl(var(--secondary))] hover:text-[hsl(var(--primary))]"
            data-testid="link-start-order"
          >
            Start an order
          </a>
        </nav>
      ...
      {open && (
        <nav className="grid gap-4 border-t border-[hsl(var(--primary-foreground)/.12)] px-5 py-5 text-sm md:hidden">
          ...
          <a
            href="#promise"
            onClick={() => setOpen(false)}
            data-testid="mobile-link-promise"
          >
            Our promise
          </a>
          ...
          <a
            ...
            data-testid="mobile-link-start-order"
          >
            Start an order
          </a>
        </nav>
  ...
  return (
    <div className="space-y-5" aria-label="Loading catalogue" data-testid="loading-catalog">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="h-16 animate-pulse rounded-xl bg-[hsl(var(--muted))]" />
        <div className="h-16 animate-pulse rounded-xl bg-[hsl(var(--muted))]" />
        <div className="h-16 animate-pulse rounded-xl bg-[hsl(var(--muted))]" />
      </div>
    </div>
...
function FieldLabel({
  ...
  return (
    <div className="mb-2 flex items-baseline justify-between gap-3">
      <label className="text-[11px] font-semibold uppercase tracking-[.13em] text-[hsl(var(--primary)/.72)]">
        {children}
      </label>
...
function OrderPanel({ catalog }: { catalog: Catalog }) {
  ...
  const currencies = (catalog.currencies ?? []).filter(
    (value): value is Currency => ['USD', 'INR'].includes(value),
  );
  ...
  const [currency, setCurrency] = useState<Currency>(
    (currencies[0] ?? 'USD') as Currency,
  );
  const [deliveryMode, setDeliveryMode] =
    useState<DeliveryMode>('standard');
  ...
  const selectedPrice =
    prices.find((price) => price.slides === slideRange) ?? prices[0];

  useEffect(() => {
    if (!subjectGroupId && groups[0]) {
      setSubjectGroupId(groups[0].id);
    }
  }, [groups, subjectGroupId]);
  ...
  return (
    <section
      id="order"
      className="relative scroll-mt-10 bg-[hsl(var(--card))] px-5 py-16 sm:px-8 lg:px-10 lg:py-24"
    >
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[minmax(0,1fr)_390px] lg:gap-20">
        <div className="max-w-2xl">
          <div className="mb-8 flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[.16em] text-[hsl(var(--accent))]">
            <span className="h-px w-8 bg-[hsl(var(--accent))]" />
            Build your brief
          </div>

          <h2 className="font-editorial text-5xl leading-[.95] tracking-[-.035em] text-[hsl(var(--primary))] sm:text-6xl">
            Tell us what you need
            <br />
            <em className="text-[hsl(var(--accent))]">
              to present well.
            </em>
          </h2>

          <p className="mt-6 max-w-lg text-[15px] leading-7 text-[hsl(var(--muted-foreground))]">
            A few considered details give our studio the right context. We
            handle the structure, the visual logic, and the last-mile polish.
          </p>

          <div className="mt-10 hidden border-l border-[hsl(var(--accent)/.4)] pl-5 text-sm leading-6 text-[hsl(var(--primary)/.68)] sm:block">
            <span className="font-editorial text-xl italic text-[hsl(var(--accent))]">
              “
            </span>{' '}
            Clear thinking deserves a clear deck.
            <br />
            <span className="ml-5 text-[11px] uppercase tracking-[.12em]">
              The PPT pls standard
            </span>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <form
            ...
            className="relative rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--background))] p-5 shadow-[var(--shadow-md)] sm:p-7]"
            data-testid="form-order"
          >
            <div className="mb-7 flex items-center justify-between border-b border-[hsl(var(--border))] pb-5">
              <div>
                <p className="font-mono-ui text-[10px] uppercase tracking-[.14em] text-[hsl(var(--accent))]">
                  01 / 04
                </p>
              ...
              <span className="rounded-full bg-[hsl(var(--secondary)/.16)] px-3 py-1 text-[10px] font-semibold uppercase tracking-[.1em] text-[hsl(var(--primary))]">
                ~ 2 min
              </span>
            </div>
            ...
            <div className="space-y-5">
              <div>
                <FieldLabel>Subject group</FieldLabel>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(event) =>
                      setSubjectGroupId(event.target.value)
                    }
                    className="form-select"
                    data-testid="select-subject-group"
                  >
                    {groups.map((group) => (
                  ...
                  <ChevronDown
                    className="pointer-events-none absolute right-4 top-3.5 text-[hsl(var(--muted-foreground))]"
                    size={16}
                  />
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    {(selectedGroup?.subjects ?? []).map((item) => (
              ...
              <div>
                <FieldLabel hint="Choose your depth">
                  Slide count
                </FieldLabel>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {prices.map((price) => (
                    <button
                      type="button"
                      key={price.id}
                      onClick={() => setSlideRange(price.slides)}
                      className={`choice-tile ${
                        selectedPrice?.id === price.id
                          ? 'choice-tile-active'
                          : ''
                      }`}
                      data-testid={`button-slide-range-${price.id}`}
                    >
                      ...
                      <span className="mt-1 block text-[10px] text-[hsl(var(--muted-foreground))]">
                        <PriceLine
                          amount={
                            price[
                              currency.toLowerCase() as 'usd' | 'inr'
                            ]
                          }
                          currency={currency}
                        />
              ...
              <div>
                <FieldLabel>Delivery preference</FieldLabel>

                <div className="grid gap-2 sm:grid-cols-2">
                  {(['standard', 'personalized'] as DeliveryMode[]).map(
                    (mode) => (
                      <button
                        type="button"
                        key={mode}
                        ...
                        data-testid={`button-delivery-${mode}`}
                      >
                        ...
                        <span className="mt-1 block pl-5 text-[11px] leading-4 text-[hsl(var(--muted-foreground))]">
                          {mode === 'standard'
                            ? 'Clear, polished, on brief'
                            : 'More direction, more refinement'}
                        </span>
                      </button>
                    ),
                  )}
              ...
              <div>
                <FieldLabel hint="At least 3 characters">
                  Topic
                </FieldLabel>
              ...
              <div>
                <FieldLabel hint="Optional">
                  Requirements or source material
                </FieldLabel>
              ...
              <div>
                <FieldLabel>Delivery email</FieldLabel>
                ...
                <div className="flex gap-2">
                  {currencies.map((item) => (
                    <button
                      ...
                      data-testid={`button-currency-${item}`}
                    >
                      {item}
                    </button>
                  ))}
            ...
            <div className="mt-7 border-t border-[hsl(var(--border))] pt-5">
              <div className="mb-4 flex items-end justify-between">
                <div>
                  <p className="text-[11px] uppercase tracking-[.13em] text-[hsl(var(--muted-foreground))]">
                    Estimated total
                  </p>

                  <p className="mt-1 font-editorial text-3xl text-[hsl(var(--primary))]">
                    <PriceLine
                      amount={
                        selectedPrice?.[
                          currency.toLowerCase() as 'usd' | 'inr'
                ...
                <p className="max-w-[135px] text-right text-[11px] leading-4 text-[hsl(var(--muted-foreground))]">
                  Final delivery timing is confirmed after your brief.
                </p>
              </div>
              ...
              <p className="mt-3 flex items-center justify-center gap-1.5 text-center text-[10px] text-[hsl(var(--muted-foreground))]">
                <ShieldCheck
                  size={12}
    ...
    <div className="site-shell min-h-[100dvh] bg-[hsl(var(--background))]">
      ...
      <main>
        <section className="relative overflow-hidden bg-[hsl(var(--primary))] px-5 pb-20 pt-16 text-[hsl(var(--primary-foreground))] sm:px-8 sm:pb-28 sm:pt-24 lg:px-10">
          <div className="absolute right-[-8%] top-[-18%] h-[540px] w-[540px] rounded-full border border-[hsl(var(--secondary)/.15)] sm:h-[720px] sm:w-[720px]" />

          <div className="absolute right-[8%] top-[12%] h-[310px] w-[310px] rounded-full border border-[hsl(var(--accent)/.18)]" />

          <div className="relative mx-auto grid max-w-7xl gap-14 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
            <div className="max-w-3xl">
              <div className="fade-up mb-8 flex items-center gap-3 font-mono-ui text-[10px] uppercase tracking-[.19em] text-[hsl(var(--secondary))]">
                <span className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--secondary))]" />
                Academic presentation studio
                <span className="ml-1 h-px w-12 bg-[hsl(var(--secondary)/.45)]" />
              </div>

              <h1 className="fade-up delay-1 font-editorial text-[clamp(4rem,10vw,8.7rem)] leading-[.82] tracking-[-.055em]">
                A better
                <br />
                <em className="text-[hsl(var(--secondary))]">
                  last minute.
                </em>
              </h1>

              <p className="fade-up delay-2 mt-9 max-w-xl text-[16px] leading-7 text-[hsl(var(--primary-foreground)/.7)] sm:text-lg">
                You bring the topic and the deadline. We bring the clinical
                clarity, editorial eye, and a deck you can present without
                apology.
              </p>

              <a
                href="#order"
                className="fade-up delay-3 mt-9 inline-flex items-center gap-3 rounded-full bg-[hsl(var(--secondary))] px-5 py-3.5 text-[13px] font-semibold text-[hsl(var(--primary))] transition-transform hover:-translate-y-0.5"
                data-testid="link-hero-start-order"
              >
            ...
            <div className="fade-up delay-3 lg:pb-2">
              <div className="ml-auto max-w-xs border-l border-[hsl(var(--secondary)/.35)] pl-5">
                <p className="font-editorial text-2xl leading-tight text-[hsl(var(--primary-foreground)/.9)]">
                  For the morning after the all-nighter.
                </p>

                <p className="mt-4 text-[11px] uppercase tracking-[.13em] text-[hsl(var(--primary-foreground)/.5)]">
                  Order in minutes · present with confidence
                </p>
              </div>
            </div>
          </div>
        ...
        <section
          id="how-it-works"
          className="paper-grid border-b border-[hsl(var(--border))] bg-[hsl(var(--card))] px-5 py-14 sm:px-8 lg:px-10"
        >
            ...
            {[
              {
                ...
                body: 'Choose your subject, slide count, and the one thing your audience must remember.',
              },
              {
                n: '02',
                icon: <Sparkles size={18} />,
                title: 'We shape the story',
                body: 'Our studio turns clinical material into a clean, coherent visual argument.',
              },
              {
                n: '03',
                icon: <FileText size={18} />,
                title: 'You present ready',
                body: 'A deck arrives in your inbox with the polish and pacing your room deserves.',
              },
            ].map((item, index) => (
              <div
                key={item.n}
                className={`fade-up delay-${index + 1} relative border-[hsl(var(--border))] md:px-8 ${
                  index < 2 ? 'md:border-r' : ''
                } ${index === 0 ? 'md:pl-0' : ''}`}
              >
                <div className="flex items-center justify-between text-[hsl(var(--accent))]">
                  <span className="font-mono-ui text-[10px] tracking-[.16em]">
                    {item.n}
                  </span>
                  {item.icon}
                </div>

                <h3 className="mt-5 font-editorial text-2xl text-[hsl(var(--primary))]">
                  {item.title}
                </h3>
        ...
        <section
          id="promise"
          className="bg-[hsl(var(--secondary))] px-5 py-16 text-[hsl(var(--primary))] sm:px-8 lg:px-10 lg:py-24"
        >
          <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
            <div>
              <p className="font-mono-ui text-[10px] uppercase tracking-[.17em] opacity-70">
                The studio promise
              </p>

              <h2 className="mt-5 max-w-md font-editorial text-5xl leading-[.93] tracking-[-.035em] sm:text-6xl">
                Quietly serious about your deadline.
              </h2>
            </div>

            <div className="grid max-w-xl gap-6 sm:grid-cols-2">
              <div className="border-t border-[hsl(var(--primary)/.25)] pt-4">
                <Clock3 size={19} />
                ...
                <p className="mt-2 text-sm leading-6 opacity-70">
                  We respect the clock without making the work feel rushed.
                </p>
              </div>
              ...
              <div className="border-t border-[hsl(var(--primary)/.25)] pt-4">
                ...
                <p className="mt-2 text-sm leading-6 opacity-70">
                  Clear hierarchy, accurate language, and nothing decorative
                  without purpose.
                </p>
              </div>
            </div>
      ...
      <footer className="bg-[hsl(var(--primary))] px-5 py-7 text-[hsl(var(--primary-foreground)/.62)] sm:px-8 lg:px-10">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 text-[11px] sm:flex-row sm:items-center sm:justify-between">
          <span className="font-semibold tracking-[-.02em] text-[hsl(var(--primary-foreground))]">
            PPT pls
          </span>

          <span>
            Presentation support for the clinically curious.
          </span>

          <a
            href="mailto:2pptpls257@gmail.com"
            className="transition-colors hover:text-[hsl(var(--secondary))]"
...
function Success() {
  ...
  return (
    <div className="site-shell min-h-[100dvh] bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]">
      <Header />

      <main className="mx-auto flex min-h-[calc(100dvh-78px)] max-w-3xl items-center px-5 py-16 sm:px-8">
        <div className="w-full">
          <div className="draw-in mx-auto grid h-16 w-16 place-items-center rounded-full border border-[hsl(var(--secondary)/.65)] text-[hsl(var(--secondary))]">
            <Check size={28} strokeWidth={1.5} />
          </div>

          <div className="fade-up delay-1 mt-8 text-center">
            <p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-[hsl(var(--secondary))]">
              Brief received
            </p>

            <h1 className="mt-5 font-editorial text-6xl leading-[.9] tracking-[-.045em] sm:text-8xl">
              You’re in
              <br />
              <em className="text-[hsl(var(--secondary))]">
                good hands.
            ...
            <p className="mx-auto mt-7 max-w-md text-[15px] leading-7 text-[hsl(var(--primary-foreground)/.68)]">
              Your request is with the studio. We will send the next update to
              your delivery email as soon as your deck is underway.
            </p>

            {orderId && (
              <p
                className="mt-7 font-mono-ui text-[10px] uppercase tracking-[.13em] text-[hsl(var(--primary-foreground)/.45)]"
            ...
            <Link
              href="/"
              className="secondary-button mt-10 border-[hsl(var(--secondary)/.6)] bg-transparent text-[hsl(var(--secondary))] hover:bg-[hsl(var(--secondary))] hover:text-[hsl(var(--primary))]"
